...
from .audio_encoder.stft_encoder import STFTResNetWithAttention,STFTResNeXtWithAttention
from .audio_encoder.wavelet_encoder import WaveletResNetWithAttention, WaveletResNeXtWithAttention
...
class MUSER(CLIP):
    ...
    def __init__(self,
        ...
        self.audio = WaveletResNeXtWithAttention(
            n_fft=n_fft,
        ...
        self.logit_scale_it = torch.nn.Parameter(torch.log(torch.ones([]) * 100))
    ...
    def encode_text(self,
        ...
        return super(MUSER, self).encode_text(text_tokens)
        #return text_tokens
    ...
    def loss_fn(self, logits: ClipLogits, sample_weights: Optional[torch.Tensor] = None) -> Optional[torch.Tensor]:
        ...
        if logits_audio_text is not None:
            loss_at = F.cross_entropy(logits_audio_text, reference, weight=sample_weights) + F.cross_entropy(logits_audio_text.transpose(-1, -2), reference, weight=sample_weights)
            loss = loss + loss_at
            num_modalities += 1
