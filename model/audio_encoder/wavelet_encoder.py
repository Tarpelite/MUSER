...
from .AttentionResNet import ResNetWithAttention, BasicBlock, Bottleneck
from .stft_encoder import _STFTResNetWithAttention
...
class LinearWavelet(nn.Module):
    ...
    def __init__(self, out_features: int, bias: bool = True, normalized: bool = False):
        ...
        self.register_parameter('bias',torch.nn.Parameter(torch.normal(0.0, 0.5, (self.out_features, 2), dtype=default_dtype) if bias else cast(torch.nn.Parameter, None)))
...
class _WaveletResNetWithAttention(_STFTResNetWithAttention):
    ...
    def loss_ttf(self, device: torch.device) -> torch.Tensor:
        ...
        loss_ttf_norm = F.mse_loss(ttf_norm, torch.full_like(ttf_norm, 1.0 if self.normalized else self.n_fft ** 0.5))

        return loss_ttf_norm

class WaveletResNetWithAttention(_WaveletResNetWithAttention):
    ...
    def __init__(self,
        ...
        super(WaveletResNetWithAttention, self).__init__(
            block=Bottleneck,
            layers=[3, 4, 6, 3],
...
class WaveletResNeXtWithAttention(_WaveletResNetWithAttention):
    ...
    def __init__(self,
        ...
        super(WaveletResNeXtWithAttention, self).__init__(
            block=Bottleneck,
            layers=[3, 4, 6, 3],
            apply_attention=apply_attention,
            n_fft=n_fft,
